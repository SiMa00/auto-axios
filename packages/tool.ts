...
import type { IObjAny, IObj } from "./types"

/**
 * iframe 内外通讯
 * @param actData { type, data? }
 * @param host string
 */
export function postFrameMsg<D, ETY>(actData:{ type: ETY, data?: D }, host:string = '*') {
...
export function getStorageVal(field:string, moduleKey = 'admin-user', storage?: 'local'|'session') {
 ...
 * @example
 *  Arr1
 * [{ key: 'name', defaultVal: 'ff' }, { key: 'age', }] =>> { name: 'ff', age: '', }
 ...
 */
export function downloadFile (file:Blob|string, fileName:string = Date.now()+'', suffixTxt?:string, openType = '_blank') {
    if (isStringVal(file)) {
